...
const seed = (px, py) => {
	...
	for (let i = 0; i < 81; ++i) if (!toRemove.includes(i)) possiblePositions.push(i);
	...
	bombs.forEach(([x, y]) => {
		getNeighbours(x, y).forEach(([nx, ny]) => {
			if (dataGrid[ny][nx] != BOMB) dataGrid[ny][nx]++;
		});
	});

	for (let y = 0; y < 9; ++y)
		for (let x = 0; x < 9; ++x) if (dataGrid[y][x] === UNKNOWN) dataGrid[y][x] = EMPTY;
...
const click = (relX, relY, btn) => {
	...
	if (btn) {
		...
		if (flags === 10) {
			let complete = true;
			bombs.forEach(([x, y]) => {
				if (grid[y][x] !== FLAG) complete = false;
			});
		...
		if (dataGrid[relY][relX] === BOMB) gameover = true;
		else reveal(relX, relY);
	}
...
const render = (win = false) => {
	...
	for (let i = 0; i < 9; ++i)
		for (let j = 0; j < 9; ++j) {
			...
			if (data === FLAG) drawFlag(G_PAD_L + j * C_DIM_GAP, G_PAD_T + i * C_DIM_GAP);

			if (data === BOMB)
				if (win) drawFlag(G_PAD_L + j * C_DIM_GAP, G_PAD_T + i * C_DIM_GAP, true);
				else drawBomb(G_PAD_L + j * C_DIM_GAP, G_PAD_T + i * C_DIM_GAP);
		}
};
...
const reveal = (x, y) => {
	...
	if (data === EMPTY) {
		getNeighbours(x, y).forEach(([nx, ny]) => {
			if (grid[ny][nx] === UNKNOWN) reveal(nx, ny);
		});
	}
};
...
const getNeighbours = (x, y) => {
	return REL.map(([dx, dy]) => [x + dx, y + dy]).filter(([nx, ny]) => valid(nx) && valid(ny));
};
